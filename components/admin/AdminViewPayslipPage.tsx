...
const AdminViewPayslipPage: React.FC<AdminViewPayslipPageProps> = ({ 
  ...
  const [targetShalarthId, setTargetShalarthId] = useState<string>('');
  ...
  const handleGeneratePayslip = () => {
    ...
    if (!targetShalarthId.trim()) {
      setError("Please enter a Shalarth ID.");
      return;
    }

    const foundTeacher = teachers.find(t => t.shalarthId === targetShalarthId.trim());
    if (!foundTeacher) {
      setError(`Teacher with Shalarth ID "${targetShalarthId}" not found. Please verify the ID.`);
      return;
    }
    ...
    const foundData = monthlySalaryDataList.find(
      data => data.month === selectedMonth && 
              ...
              data.teacherShalarthId === foundTeacher.shalarthId
    );
    ...
    if (foundData) {
      ...
      setError(`Payslip data for ${selectedMonth} ${selectedYear} is not available for Shalarth ID ${foundTeacher.shalarthId} (${foundTeacher.name}). Please contact Admin at ${adminContactMobile}.`);
    }
  };
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
            <div className="lg:col-span-1">
              ...
              <input 
                type="text"
                id="adminShalarthId"
                value={targetShalarthId}
                onChange={e => setTargetShalarthId(e.target.value)}
      ...
      {generatedSalaryData && targetTeacher && (
        <PayslipDisplay 
